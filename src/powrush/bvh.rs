...
impl Aabb {
    ...
    pub fn intersects_ray(&self, origin: Vector3<f64>, dir: Vector3<f64>) -> bool {
        ...
        let mut t1 = (self.min.x - origin.x) * inv.x;
        let mut t2 = (self.max.x - origin.x) * inv.x;
        if t1 > t2 { std::mem::swap(&mut t1, &mut t2); }

        let mut t3 = (self.min.y - origin.y) * inv.y;
        let mut t4 = (self.max.y - origin.y) * inv.y;
        if t3 > t4 { std::mem::swap(&mut t3, &mut t4); }
        if t1 > t4 || t3 > t2 { return false; }

        let mut t5 = (self.min.z - origin.z) * inv.z;
        let mut t6 = (self.max.z - origin.z) * inv.z;
...
impl Bvh {
    pub fn from_spheres_top_down(centers: &[Vector3<f64>], radii: &[f64]) -> Self {
        ...
        fn build(
            ...
            // Choose best axis using SAH
            let mut best_axis = 0;
            let mut best_cost = f64::INFINITY;

            for axis in 0..3 {
                let mut sorted_indices = indices.to_vec();
                sorted_indices.sort_by(|&a, &b| centers[a][axis].partial_cmp(&centers[b][axis]).unwrap());
                let mid = sorted_indices.len() / 2;
                let (lefts, rights) = sorted_indices.split_at(mid);
