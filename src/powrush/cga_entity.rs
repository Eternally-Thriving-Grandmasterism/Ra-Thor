...
impl CgaEntity {
    ...
    /// Currently uses point-to-sphere distance (future: full CGA meet).
    ...
    /// Treats entity as a point for now (future: full CGA geometric product).
    pub fn intersects_plane(&self, plane: &CgaPlane) -> bool {
        let pos = self.world_position().to_euclidean();
        let dist = plane.normal.dot(&pos) - plane.distance;
        dist.abs() <= 1e-6 // small epsilon for floating point safety
    }
}