# src/ml/gesture_transformer/quantize_post_training.py – Post-Training Quantization v1.0
...
def calibrate_model(model: nn.Module, loader: DataLoader, num_batches: int = 100):
    ...
    with torch.no_grad():
        for i, (x, _, _) in enumerate(loader):
            if i >= num_batches:
                break
            model(x)  # forward pass to collect stats
...
def main(args):
    ...
    print(f"[PTQ] Using device: {device}")
    ...
    # Calibration dataset – high-valence sequences (replace with real data)
    ...
    y_valence_calib = torch.rand(num_calib_samples, args.future_horizon) * 0.8 + 0.15  # bias toward high valence
    ...
    # Fuse modules (Conv+BN, Linear+ReLU, etc.) – required for QAT/PTQ
    model.fuse_modules()  # implement in GestureTransformer if needed

    # Apply PTQ
    quantized_model = calibrate_model(model, calib_loader, num_batches=args.calib_batches)

    # Evaluate before/after (dummy accuracy check)
    quantized_model.eval()
    with torch.no_grad():
        dummy_input = torch.randn(4, args.seq_len, args.landmark_dim).to(device)
        orig_out = model(dummy_input)
        q_out = quantized_model(dummy_input)

        diff = torch.abs(orig_out[0] - q_out[0]).mean().item()
        print(f"[PTQ] Mean absolute difference after quantization: {diff:.6f}")
    ...
    print(f"[PTQ] Done – INT8/INT4 model ready for WebNN/TensorRT inference")
