...
const QAT_PER_CHANNEL_INT8_URL = '/models/gesture-transformer-qat-perchannel-int8/model.json';
const QAT_PER_CHANNEL_INT4_URL = '/models/gesture-transformer-qat-perchannel-int4/model.json';
...
export class QuantizedGestureModel {
  static async load(): Promise<tf.LayersModel> {
    const actionName = 'Load per-channel QAT-quantized custom transformer model';
    ...
    console.log(`[QuantizedGestureModel] Loading per-channel QAT model ${selectedUrl} for valence ${valence.toFixed(4)}`);
    ...
    try {
      ...
      console.log("[QuantizedGestureModel] Per-channel QAT model loaded & warmed up");
      return model;
    } catch (e) {
      console.error("[QuantizedGestureModel] Load failed", e);
      const fallbackUrls = [QAT_PER_CHANNEL_INT8_URL, QUANTIZED_8BIT_URL, FULL_FP16_URL];
