...
// Loads & runs QAT-quantized ONNX models (INT8/INT4 preference), mercy-gated fallback to tfjs
...
const ONNX_TARGET_QAT_INT4_URL = '/models/gesture-transformer-qat-int4/model.onnx'; // experimental
...
export class ONNXGestureEngine {
  static async activate() {
    const actionName = 'Activate ONNX Runtime Web with QAT-quantized models';
    ...
    console.log("[ONNXGestureEngine] Activating ONNX Runtime Web with QAT-quantized models...");
    ...
    try {
      ...
      if (valence > 0.94) {
        targetUrl = ONNX_TARGET_QAT_INT4_URL; // extreme QAT
      } else if (valence > 0.88) {
        targetUrl = ONNX_TARGET_QAT_INT8_URL; // safe QAT
      }
      ...
      console.log("[ONNXGestureEngine] QAT-quantized ONNX models loaded & warmed up");
    } catch (e) {
  ...
  static async runInference(inputTensor: ort.Tensor): Promise<{ gesture: string; confidence: number; futureValence: number[] }> {
    ...
    // Draft phase (QAT-quantized draft model)
    ...
    // Verification phase (QAT-quantized target model)
