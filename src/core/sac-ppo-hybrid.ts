// src/core/sac-ppo-hybrid.ts – SAC + PPO Hybrid Engine v1.0
...
// Mercy gating, continuous control, lattice-integrated training loop
...
export class SACPPOHybrid {
  ...
  async collectTrajectory(maxSteps: number = MAX_TRAJECTORY_LENGTH): Promise<Transition[]> {
    ...
    for (let step = 0; step < maxSteps; step++) {
      ...
      const logProb = Math.log(actionProbs[actionIndex]);
      ...
      trajectory.push({
        ...
        action,
        oldLogProb: logProb,
        newLogProb: logProb, // will be updated on next policy eval
        value,
        reward,
        nextState,
        done,
