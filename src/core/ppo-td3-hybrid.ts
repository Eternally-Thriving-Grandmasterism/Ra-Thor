// src/core/ppo-td3-hybrid.ts – PPO + TD3 Hybrid Engine v1.0
// Proximal Policy Optimization + Twin Delayed Deep Deterministic Policy Gradient fusion
// Clipped surrogate objective + clipped double Q-learning + delayed policy updates
// Valence-shaped advantage & target smoothing noise, mercy gating, continuous control
...
interface Transition {
  ...
  valence: number;
  logProb?: number;                       // from PPO stochastic policy (optional)
}
...
export class PPOTD3Hybrid {
  private actor: {
    predictAction: (state: any) => Promise<any>;              // deterministic μ(s)
  ...
  async collectTrajectory(maxSteps: number = MAX_TRAJECTORY_LENGTH): Promise<Transition[]> {
    ...
    for (let step = 0; step < maxSteps; step++) {
      ...
      // Optional stochastic log-prob for PPO entropy (if actor is stochastic)
      const logProb = await this.getLogProb(state, action); // optional
  ...
  private async getLogProb(state: any, action: any): Promise<number | undefined> {
    // Optional – if actor is stochastic, compute log-prob
    // Otherwise return undefined for pure TD3
    return undefined;
  }
  ...
  async update() {
    ...
    if (policyUpdateCounter % TD3_POLICY_DELAY === 0) {
      // PPO-style clipped surrogate for actor
      const stats = await this.computePPOLoss(
