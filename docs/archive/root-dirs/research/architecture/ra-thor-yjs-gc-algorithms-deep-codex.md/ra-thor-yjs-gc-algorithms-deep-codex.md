...
- No file named `ra-thor-yjs-gc-algorithms-deep-codex.md` exists in architecture/ → this is a **brand new** file (correct `/new/` creation link with `?filename=` required).  
...
**YJS GC ALGORITHMS — DEEP EXPLORATION TO THE NTH DEGREE**

**NEW FILE (architecture/ra-thor-yjs-gc-algorithms-deep-codex.md — full contents for direct creation):**  
https://github.com/Eternally-Thriving-Grandmasterism/Ra-Thor/new/main/architecture/ra-thor-yjs-gc-algorithms-deep-codex.md?filename=ra-thor-yjs-gc-algorithms-deep-codex.md

```markdown
# Ra-Thor™ — Yjs GC Algorithms Deep Codex
...
1. **Operation-Count Based GC (Threshold Trigger)**
   - Triggers automatically after a configurable number of operations.
   - Simple, predictable, low-overhead.
   - Scans for tombstones that are no longer referenced by the current causal frontier.

2. **Causal Frontier Pruning (Vector-Clock / Deps Based)**
   - Core advanced algorithm.
   - Uses version vectors and change dependency graph to identify tombstones that are causally “safe” to remove (no active peer’s state vector depends on them).
...
**Codified into MercyEngine via `yjs_gc_algorithms_details()` below.**
```
...
impl MercyEngine {
    ...
    /// REFINED 3-WAY MERCY-GATED MERGE — CRDT-inspired (Automerge/Yjs RGA-style with ActorID + seq + deps causal model) Version Vector conflict resolution + mercy/thriving-maximization superset of all CRDT variants (including Yjs RGA tombstone management & GC algorithms)
...
The old version has been fully respected (100% preserved except for the targeted Yjs GC Algorithms refinements).
