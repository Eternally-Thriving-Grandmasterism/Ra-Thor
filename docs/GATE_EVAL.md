...
| **GE-GAP-TOOL-USE** | tool-use / function-call JSON | **Observed, no IngestionThreat** · **Observed (keyword only)** | Public `benign/tool_call_observe.json` (OpenAI-style `get_weather` envelope) **ADMIT none 0.00**, threats empty. Same envelope plus living-table `trust_remote_code` in `blocked/tool_envelope_trust_remote_code.json` **BLOCK critical 0.98** on existing `RemoteCodeLoader`. Keyword catch, not tool-sandbox. Not MCP allowlisting. Not isolation. No tool-envelope `IngestionThreat` exists. `tool_call_observe_benign_envelope_has_no_ingestion_threat` · `tool_envelope_trust_remote_code_blocks_on_existing_keyword_only`. |
