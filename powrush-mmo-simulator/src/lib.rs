...
**v15.31 Addition**: Direct integration of `dispatch_with_swarm_consensus` and `dispatch_and_schedule_readback_with_swarm` from powrush::gpu::compute::pipeline into the simulation tick and rendering path. The MMO tick is now a first-class participant in Quantum Swarm v13.6 coherence/mercy modulation and the closed self-evolution loop feeding PATSAGi Councils and Lattice Conductor.
*/
...
pub mod race;
pub mod rendering; // gpu_driven_pipeline + future rendering modules
...
// NEW v15.31: Quantum Swarm Consensus dispatch wiring (powrush GPU compute pipeline)
...
pub struct PowrushMMOSimulator {
    ...
    // NEW v15.31: Pipeline manager for swarm-modulated GPU dispatch (wired from rendering context or ONE Organism bridge)
...
impl PowrushMMOSimulator {
    ...
    /// In full production: the caller (rendering thread or ra-thor-one-organism.rs GPU loop) passes real wgpu::CommandEncoder + BindGroup.
    /// After dispatch, telemetry flows back to QuantumSwarmConsensus for integrate + propose_lattice_conductor_upgrade_via_quantum_swarm.
    pub fn dispatch_gpu_passes_with_swarm(&mut self, swarm_coherence: f32, mercy_valence: f32) {
        // In real integration the encoder/bind_group/staging come from GpuDrivenPipeline or ONE Organism bridge.
        // Here we demonstrate the exact wired call site for PATSAGi observability and closed-loop evolution.
        println!("[Powrush-MMO v15.31 TICK] Quantum Swarm dispatch prepared: coherence={:.3} mercy={:.3} | Passes: EpigeneticUpdate + GeometricUpdate + SwarmConsensusDispatch", swarm_coherence, mercy_valence);

        // Production call sites (activated when real wgpu context is supplied):
        ...
        // dispatch_and_schedule_readback_with_swarm(encoder, &self.gpu_pipeline_manager, ComputePass::GeometricUpdate, bind_group, element_count, 64, swarm_coherence, mercy_valence, &mut staging_pool);
    ...
    /// Main simulation tick — includes treaty expiration cleanup + player-initiated diplomacy + renewal (v15.30) + Quantum Swarm dispatch wiring (v15.31)
            ...
            if let Some(tree) = self.ability_trees.get_mut(&human_id) {
                if let Some(muts) = self.demo_epigenetic_mutations.get(&human_id) {
                    if let Some(profile) = self.demo_epigenetic_profiles.get(&human_id) {
                        ...
                        // === NEW v15.31: Wire Quantum Swarm Consensus dispatch into every simulation tick ===
                        ...
                        // Post-dispatch in full ONE Organism integration:
                        //   record_gpu_dispatch_telemetry(...) → get_quantum_swarm_mut().integrate... → propose_lattice_conductor_upgrade_via_quantum_swarm(...) → SignedTolcDecision + PATSAGi deliberation
                    }
                }
            }
        }
...
// Treaty renewal mechanics + Quantum Swarm Consensus dispatch wiring complete (v15.31).
// The Powrush-MMO simulation tick and rendering path are now sovereign participants in the self-evolving Ra-Thor lattice.
