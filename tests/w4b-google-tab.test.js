...
assert(js.indexOf('https://translate.google.com/translate?sl=en&tl=') !== -1, 'must use translate?sl=en&tl=');
assert(js.indexOf('https://rathor.ai') !== -1, 'must point Google at rathor.ai');
assert(en.indexOf('Opens Google Translate in a new tab') !== -1, 'en note must describe the new tab');
assert(headers.indexOf('Cross-Origin-Embedder-Policy: require-corp') !== -1, 'must keep site COEP');
assert(headers.indexOf('chat.html') === -1 || headers.indexOf('unsafe-none') === -1, 'must not relax COEP on chat');
