// neat-engine.js – sovereign client-side Neuroevolution of Augmenting Topologies (NEAT)
...
class NeuronGene {
  constructor(id, type = "hidden") {
    ...
    this.activation = Math.tanh; // simple activation
  }
}
...
class NEAT {
  constructor(inputs = 4, outputs = 1, popSize = 150, maxGenerations = 100) {
    ...
    this.c1 = 1.0; // excess genes
    this.c2 = 1.0; // disjoint genes
    this.c3 = 0.4; // weight difference
  ...
  initPopulation() {
    for (let i = 0; i < this.popSize; i++) {
      ...
      // Initial minimal topology: all inputs connected to all outputs
  ...
  async evolve(fitnessFunction) {
    for (let gen = 0; gen < this.maxGenerations; gen++) {
      ...
      for (const species of this.species) {
        ...
        const elite = species.genomes[0]; // elitism
  ...
  speciate() {
    ...
    for (const genome of this.population) {
      let found = false;
      for (const species of this.species) {
        const representative = species.genomes[0];
        if (this.compatibilityDistance(genome, representative) < this.compatibilityThreshold) {
  ...
  compatibilityDistance(g1, g2) {
    // Simplified compatibility distance (excess + disjoint + weight diff)
  ...
  tournamentSelect() {
    // Simplified – select from fittest species
