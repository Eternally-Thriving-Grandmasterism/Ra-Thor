...
(function () {
  ...
  function applyLockI18n(lang) {
    ...
    if (!t) return;
    document.querySelectorAll('#living-surfaces [data-i18n], #science-lattices [data-i18n], #indexed-lattice [data-i18n], #faq-q9, #faq-q10, [data-lock-i18n]').forEach(function (el) {
  ...
  ready(function () {
      ...
      if (cta) {
        cta.insertAdjacentHTML(
          ...
          '<section id="living-surfaces" class="max-w-4xl mx-auto px-6 pb-8">' +
            ...
            '<p class="text-sm text-white/70 leading-relaxed" data-i18n="mapBody">Full-repo awareness locked 22 Aug 2026. Cargo workspace 14.15.6 is the version source of truth. Lattice Chat surface is v14.18.0. GPU / tick path is documented in v15.34 notes without bumping workspace identity. Cosmic Loop remains mandatory identity.</p>' +
            '</div></section>'
        );
      }
    }
    ...
    if (!document.getElementById('faq-q9')) {
      ...
      if (acc) {
        acc.insertAdjacentHTML(
          'beforeend',
          '<details><summary class="flex items-center gap-3"><span class="lightning-icon text-amber-300">⚡</span> <span id="faq-q9" data-i18n="faqQ9">What is Lattice Chat?</span> <span class="text-3xl">⚡</span></summary><div id="faq-a9" class="faq-content mt-6 text-white/80 leading-relaxed">Privacy-first multi-session chat at /chat.html. Offline-first, no login, no backend we control. v14.18.0 adds optional passphrase encryption via Web Crypto.</div></details>' +
            '<details><summary class="flex items-center gap-3"><span class="lightning-icon text-amber-300">⚡</span> <span id="faq-q10" data-i18n="faqQ10">Are the fusion and superconductor repos part of Ra-Thor?</span> <span class="text-3xl">⚡</span></summary><div id="faq-a10" class="faq-content mt-6 text-white/80 leading-relaxed">Sister public discovery lattices under the same steward, AG-SML v1.0, and TOLC 8. Research surfaces — not a working plant or commercial superconductor product.</div></details>'
        );
      }
    }
    ...
    console.info('[Ra-Thor] site-lock-2026-08-22 applied (i18n-aware)');
  });
})();
