...
/// 
/// Includes structured logging via tracing (info + trace levels).
...
/// with robust regex-based JSON extraction, output validation, and structured logging.
#[tracing::instrument(skip(model), fields(proposal_len = proposal.len()))]
pub fn evaluate_proposal_with_tolc_and_mercy(
    model: &LlamaModel,
    proposal: &str,
) -> EvaluationResult {
    info!("Starting proposal evaluation");
    trace!(raw_response_len = raw_response.len(), "Received raw response from LLM");
...
{{
  ...
  "detailed_feedback": "Key observations, strengths, and concerns"
    ...
    trace!(extracted_json_len = json_str.len(), "JSON extracted from response");

    match serde_json::from_str::<serde_json::Value>(&json_str) {
        Ok(json) => {
            if !validate_scores(&json) {
                warn!("Evaluation validation failed - invalid scores detected");
            ...
            trace!(
                ...
                compassion = result.compassion_score,
                sovereignty = result.sovereignty_gate_score,
                non_harm = result.non_harm_gate_score,
                "Detailed scores logged at trace level"
            );

            result
        }
        Err(_) => {
            warn!("Failed to parse LLM evaluation response as JSON");
            EvaluationResult {
                truth_score: 0.0,
                order_score: 0.0,
                logic_score: 0.0,
