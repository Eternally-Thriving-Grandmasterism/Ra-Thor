...
impl MercyEngine {
    ...
    // === NEW: evaluate_action wrapper (used across the entire monorepo) ===
    pub async fn evaluate_action(
        ...
        cehi_score: f64,
        mercy_influence: f64,
    ) -> Result<f64, MercyError> {
        let input = format!("{} | Type: {} | CEHI: {:.2}", action_description, action_type, cehi_score);
        let valence = self.compute_valence(&input).await?;
        let adjusted_valence = (valence * mercy_influence).clamp(0.0, 1.0);
        
        if adjusted_valence < 0.88 {
            return Err(MercyError::Veto(adjusted_valence));
        }
