...
impl SelfImprovementEngine {
    ...
    /// Real mercy gate check using mercy_merlin_engine
    pub async fn check_mercy_gate(&self) -> Result<MercyGateResult, Box<dyn std::error::Error + Send + Sync>> {
        ...
        Ok(MercyGateResult {
            ...
            rejection_reason: if !passed {
                Some(format!(
                    "Valence: {:.4}, Council Approved: {}",
                    valence, consensus_result.approved
                ))
            } else {
                None
            },
    ...
    pub async fn generate_improvement_proposals(
        ...
        // === Gather real data from CrateAnalyzer (Step 3) ===
        let health_reports = self.crate_analyzer.analyze_critical_crates().await?;

        let mut proposals = Vec::new();

        // Intelligent, data-driven proposal generation
        for report in health_reports {
            if report.mercy_integration_score < 6 && report.importance >= 7 {
                proposals.push(self.create_proposal(
                    &report,
                    ImprovementType::MercyIntegration,
                    "Add proper mercy_merlin_engine checks and valence gating",
                    9, 10, 5,
                ));
            }

            if report.technical_debt_score > 7 && report.importance >= 8 {
                proposals.push(self.create_proposal(
                    &report,
                    ImprovementType::ArchitectureImprovement,
                    "Refactor to reduce technical debt and improve maintainability",
                    8, 8, 6,
                ));
            }

            if report.test_coverage_score < 5 && report.importance >= 8 {
                proposals.push(self.create_proposal(
                    &report,
                    ImprovementType::Testing,
                    "Significantly improve test coverage and integration tests",
                    8, 9, 4,
                ));
            }
        }
        ...
        let mut high_quality: Vec<ImprovementProposal> = proposals
            .into_iter()
    ...
    fn create_proposal(
        ...
        ImprovementProposal {
            ...
            description: format!(
                "{} (Current mercy score: {}, debt: {}, tests: {})",
                action, report.mercy_integration_score, report.technical_debt_score, report.test_coverage_score
            ),
            expected_impact: impact,
            mercy_alignment: mercy,
            implementation_difficulty: difficulty,
            priority_score: 0.0,
            rationale: format!("High importance crate ({}) with identified weakness.", report.importance),
        }
    }

    fn calculate_priority_score(proposal: &ImprovementProposal) -> f64 {
        (proposal.expected_impact as f64 * 0.40)
            + (proposal.mercy_alignment as f64 * 0.35)
            + ((10 - proposal.implementation_difficulty) as f64 * 0.25)
    }
}