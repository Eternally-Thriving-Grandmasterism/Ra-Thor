//! Generational Starship Engine — SREL v0.5.21
...
pub struct GenerationalStarshipRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GenerationalStarshipReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct GenerationalStarshipEngine {
...
impl GenerationalStarshipEngine {
    pub fn new() -> Self {
        Self {
    ...
    pub async fn evaluate(&self, request: &GenerationalStarshipRequest, game: &mut PowrushGame) -> GenerationalStarshipReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            game.boost_faction_joy(Faction::HarmonyWeavers, total_joy + (request.crew_size as f64 * 3.5));

            let report = GenerationalStarshipReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 GENERATIONAL STARSHIP APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
                    ...
                    request.location, request.radiation_flux, total_energy, best_mat, elec_risk.overall_survival * 100.0, avg_valence, total_joy, request.crew_size, request.generations
                ),
            };

            info!("Rathor.ai: Generational starship mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            GenerationalStarshipReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
