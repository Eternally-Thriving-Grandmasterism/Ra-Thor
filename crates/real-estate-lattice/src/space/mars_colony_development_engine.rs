//! Mars Colony Development Engine — SREL v0.5.21
...
pub struct MarsColonyDevelopmentRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MarsColonyDevelopmentReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct MarsColonyDevelopmentEngine {
...
impl MarsColonyDevelopmentEngine {
    pub fn new() -> Self {
        Self {
    ...
    pub async fn evaluate(&self, request: &MarsColonyDevelopmentRequest, game: &mut PowrushGame) -> MarsColonyDevelopmentReport {
        let (best_mat, props, _score) = self.materials.select_optimal_material(
            RadiationType::SolarFlare, request.radiation_flux, request.current_cehi, "Mars"
        );

        let elec_risk = self.electronics.calculate_electronics_risk(
            RadiationType::SolarFlare, request.radiation_flux, &best_mat, 1.0, request.current_cehi, "Mars"
        );

        let reports = self.mapping
            .process_radiation_with_7_gates_nth_degree(
                RadiationType::SolarFlare, request.radiation_flux, "Mars", request.current_cehi, game
            )
            .await;
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            ...
            let report = MarsColonyDevelopmentReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 MARS COLONY DEVELOPMENT APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
            ...
            info!("Rathor.ai: Mars colony mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            MarsColonyDevelopmentReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
