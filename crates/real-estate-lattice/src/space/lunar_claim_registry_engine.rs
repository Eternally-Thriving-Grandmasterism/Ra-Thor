//! Lunar Claim Registry Engine — SREL v0.5.21
...
pub struct LunarClaimRegistryRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LunarClaimRegistryReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct LunarClaimRegistryEngine {
...
impl LunarClaimRegistryEngine {
    ...
    pub async fn evaluate(&self, request: &LunarClaimRegistryRequest, game: &mut PowrushGame) -> LunarClaimRegistryReport {
        let (best_mat, props, _score) = self.materials.select_optimal_material(
            RadiationType::VanAllenBelt, request.radiation_flux, request.current_cehi, "Lunar"
        );

        let elec_risk = self.electronics.calculate_electronics_risk(
            RadiationType::VanAllenBelt, request.radiation_flux, &best_mat, 1.0, request.current_cehi, "Lunar"
        );

        let reports = self.mapping
            .process_radiation_with_7_gates_nth_degree(
                RadiationType::VanAllenBelt, request.radiation_flux, "Lunar", request.current_cehi, game
            )
            .await;
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            ...
            let report = LunarClaimRegistryReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 LUNAR CLAIM REGISTRY APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
            ...
            info!("Rathor.ai: Lunar claim mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            LunarClaimRegistryReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
