//! Interstellar Probe Engine — SREL v0.5.21
...
pub struct InterstellarProbeRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InterstellarProbeReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct InterstellarProbeEngine {
...
impl InterstellarProbeEngine {
    ...
    pub async fn evaluate(&self, request: &InterstellarProbeRequest, game: &mut PowrushGame) -> InterstellarProbeReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            ...
            let report = InterstellarProbeReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 INTERSTELLAR PROBE APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
            ...
            info!("Rathor.ai: Interstellar probe mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            InterstellarProbeReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
