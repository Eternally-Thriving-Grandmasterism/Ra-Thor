//! Fusion Drive Propulsion Engine — SREL v0.5.21
...
pub struct FusionDrivePropulsionRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FusionDrivePropulsionReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct FusionDrivePropulsionEngine {
...
impl FusionDrivePropulsionEngine {
    pub fn new() -> Self {
        Self {
    ...
    pub async fn evaluate(&self, request: &FusionDrivePropulsionRequest, game: &mut PowrushGame) -> FusionDrivePropulsionReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            game.boost_faction_joy(Faction::HarmonyWeavers, total_joy + (request.thrust_kn * 1.2));

            let report = FusionDrivePropulsionReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 FUSION DRIVE PROPULSION APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
                     Thrust: {:.1} kN | Isp: {} s | Fuel: {}\n\
                    ...
                    request.location, request.thrust_kn, request.specific_impulse_s, request.fuel_type,
                    request.radiation_flux, total_energy, best_mat, elec_risk.overall_survival * 100.0, avg_valence, total_joy
                ),
            };

            info!("Rathor.ai: Fusion Drive Propulsion mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            FusionDrivePropulsionReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
