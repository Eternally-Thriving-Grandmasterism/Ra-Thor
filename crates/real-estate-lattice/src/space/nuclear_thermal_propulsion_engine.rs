//! Nuclear Thermal Propulsion Engine — SREL v0.5.21
...
impl NuclearThermalPropulsionEngine {
    ...
    pub async fn evaluate(&self, request: &NuclearThermalPropulsionRequest, game: &mut PowrushGame) -> NuclearThermalPropulsionReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            game.boost_faction_joy(Faction::HarmonyWeavers, total_joy + (request.thrust_kn * 0.8));

            let report = NuclearThermalPropulsionReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 NUCLEAR THERMAL PROPULSION APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
                     Thrust: {:.1} kN | Isp: {} s | Flux: {:.2} → {:.2} energy\n\
                     Optimal Material: {:?} | Electronics Survival: {:.1}%\n\
                     Average Gate Valence: {:.2} | Joy: +{:.1} | CEHI +0.18 (5-gen)\n\
                     13+ PATSAGi Councils: APPROVED ✓\n\
                     All crew + electronics thriving + radiation alchemized into abundance.",
                    request.location, request.thrust_kn, request.specific_impulse_s, request.radiation_flux, total_energy, best_mat, elec_risk.overall_survival * 100.0, avg_valence, total_joy
                ),
            };

            info!("Rathor.ai: Nuclear Thermal Propulsion mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            NuclearThermalPropulsionReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
