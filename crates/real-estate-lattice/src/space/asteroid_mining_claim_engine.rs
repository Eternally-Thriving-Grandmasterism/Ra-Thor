//! Asteroid Mining Claim Engine — SREL v0.5.21
...
pub struct AsteroidMiningClaimRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AsteroidMiningClaimReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct AsteroidMiningClaimEngine {
...
impl AsteroidMiningClaimEngine {
    pub fn new() -> Self {
        Self {
    ...
    pub async fn evaluate(&self, request: &AsteroidMiningClaimRequest, game: &mut PowrushGame) -> AsteroidMiningClaimReport {
        let (best_mat, props, _score) = self.materials.select_optimal_material(
            RadiationType::CosmicRays, request.radiation_flux, request.current_cehi, "Asteroid"
        );

        let elec_risk = self.electronics.calculate_electronics_risk(
            RadiationType::CosmicRays, request.radiation_flux, &best_mat, 1.0, request.current_cehi, "Asteroid"
        );

        let reports = self.mapping
            .process_radiation_with_7_gates_nth_degree(
                RadiationType::CosmicRays, request.radiation_flux, "Asteroid", request.current_cehi, game
            )
            .await;
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            ...
            let report = AsteroidMiningClaimReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 ASTEROID MINING CLAIM APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
            ...
            info!("Rathor.ai: Asteroid mining claim mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            AsteroidMiningClaimReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
