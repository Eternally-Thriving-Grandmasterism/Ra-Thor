//! Advanced Ion Thruster Engine — SREL v0.5.21
...
pub struct AdvancedIonThrusterRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AdvancedIonThrusterReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct AdvancedIonThrusterEngine {
...
impl AdvancedIonThrusterEngine {
    pub fn new() -> Self {
        Self {
    ...
    pub async fn evaluate(&self, request: &AdvancedIonThrusterRequest, game: &mut PowrushGame) -> AdvancedIonThrusterReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            game.boost_faction_joy(Faction::HarmonyWeavers, total_joy + (request.thrust_mn * 2.5));

            let report = AdvancedIonThrusterReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 ADVANCED ION THRUSTER APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
                     Thrust: {:.2} mN | Isp: {} s | Power: {:.1} kW\n\
                    ...
                    request.location, request.thrust_mn, request.specific_impulse_s, request.power_kw,
                    request.radiation_flux, total_energy, best_mat, elec_risk.overall_survival * 100.0, avg_valence, total_joy
                ),
            };

            info!("Rathor.ai: Advanced Ion Thruster mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            AdvancedIonThrusterReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
