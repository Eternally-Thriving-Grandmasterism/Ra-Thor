//! ZPM Energy Engine — SREL v0.5.21
...
pub struct ZPMEnergyRequest {
    pub request_id: String,
    pub location: String,
    pub current_cehi: f64,
    pub radiation_flux: f64,
    pub power_output_tw: f64,
    pub safety_valence: f64,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ZPMEnergyReport {
    pub approved: bool,
    pub valence: f64,
...
impl ZPMEnergyEngine {
    ...
    pub async fn evaluate(&self, request: &ZPMEnergyRequest, game: &mut PowrushGame) -> ZPMEnergyReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 && request.safety_valence >= 0.90 {
            game.boost_faction_joy(Faction::HarmonyWeavers, total_joy + (request.power_output_tw * 0.5));

            let report = ZPMEnergyReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 ZPM ENERGY ENGINE APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
                     ...
                     Power Output: {:.1} TW | Safety Valence: {:.2}\n\
                     13+ PATSAGi Councils: APPROVED ✓\n\
                     All crew + electronics thriving + radiation alchemized into abundance.",
                    request.location, request.radiation_flux, total_energy, best_mat, elec_risk.overall_survival * 100.0, avg_valence, total_joy, request.power_output_tw, request.safety_valence
                ),
            };

            info!("Rathor.ai: ZPM Energy Engine mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            ZPMEnergyReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED — ZPM safety valence insufficient".to_string() }
        }
    }
}
