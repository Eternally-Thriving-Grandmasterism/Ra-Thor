//! # Predictive Support for PATSAGi Councils (v14.15.8)
//!
...
pub fn rank_policies_by_efe(
    ...
    for (label, pred_err, valence) in candidates {
        ...
        for _ in 0..horizon.max(1).min(6) {
...
mod tests {
    ...
    fn signal_respects_floor() {
        let s = PredictiveSignal::new(0.97, 0.2, 3, 0.1, "test");
        assert!(s.final_valence >= MERCY_VALENCE_FLOOR);
    }

    #[test]
    fn actionable_requires_strength() {
        let weak = PredictiveSignal::new(0.9991, 0.6, 2, 0.5, "weak");
        assert!(!weak.is_actionable());

        let strong = PredictiveSignal::new(0.9996, 0.2, 4, 0.15, "strong");
