//! Ancient Drone Weapons Engine — SREL v0.5.21
...
pub struct AncientDroneWeaponsRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AncientDroneWeaponsReport {
    pub approved: bool,
    pub valence: f64,
...
pub struct AncientDroneWeaponsEngine {
...
impl AncientDroneWeaponsEngine {
    pub fn new() -> Self {
        Self {
    ...
    pub async fn evaluate(&self, request: &AncientDroneWeaponsRequest, game: &mut PowrushGame) -> AncientDroneWeaponsReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 && request.targeting_precision >= 0.90 {
            game.boost_faction_joy(Faction::HarmonyWeavers, total_joy + (request.drone_count as f64 * 4.2));

            let report = AncientDroneWeaponsReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 ANCIENT DRONE WEAPONS APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
                     ...
                     Drone Count: {} | Targeting Precision: {:.2}\n\
                     13+ PATSAGi Councils: APPROVED ✓\n\
                     All crew + electronics thriving + radiation alchemized into abundance.",
                    request.location, request.radiation_flux, total_energy, best_mat, elec_risk.overall_survival * 100.0, avg_valence, total_joy, request.drone_count, request.targeting_precision
                ),
            };

            info!("Rathor.ai: Ancient Drone Weapons mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            AncientDroneWeaponsReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED — Targeting precision or radiation protection insufficient".to_string() }
        }
    }
}
