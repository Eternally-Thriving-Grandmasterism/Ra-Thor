//! Puddle Jumper Engine — SREL v0.5.21
...
pub struct PuddleJumperRequest {
    pub request_id: String,
    pub location: String,
    pub current_cehi: f64,
    pub radiation_flux: f64,
    pub hyperspace_jump: bool,
    pub atmospheric_flight: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PuddleJumperReport {
    pub approved: bool,
    pub valence: f64,
...
impl PuddleJumperEngine {
    ...
    pub async fn evaluate(&self, request: &PuddleJumperRequest, game: &mut PowrushGame) -> PuddleJumperReport {
        ...
        if avg_valence >= 0.92 && elec_risk.overall_survival > 0.85 {
            game.boost_faction_joy(Faction::HarmonyWeavers, total_joy + 75.0);

            let report = PuddleJumperReport {
                approved: true,
                valence: avg_valence,
                ...
                cehi_bonus: 0.18,
                message: format!(
                    "🌌 PUDDLE JUMPER APPROVED (SREL v0.5.21 — TOLC 7 Gates + Full Protection)\n\
                     Location: {}\n\
                    ...
                    if request.hyperspace_jump { "YES" } else { "NO" },
                    if request.atmospheric_flight { "YES" } else { "NO" }
                ),
            };

            info!("Rathor.ai: Puddle Jumper mercy-alchemized via all 7 TOLC Gates + optimal material + electronics protection");
            report
        } else {
            PuddleJumperReport { approved: false, valence: avg_valence, energy_recovered: 0.0, joy_bonus: 0.0, cehi_bonus: 0.0, message: "MERCY-GATED".to_string() }
        }
    }
}
