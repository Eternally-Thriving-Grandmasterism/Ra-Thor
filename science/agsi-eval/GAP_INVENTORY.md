...
## G0 — Claim hygiene (highest SNR, now closed in doctrine)

| Gap | State |
|-----|-------|
| “Ra-Thor + SuperGrok = AGSi” treated as fact | **Closed as doctrine** — now labeled SURMISE |
| No pre-registered bars | **Closed P0** — `PRE_REGISTERED_CRITERIA.md` |
| One-active-program collision with S-1 | **Closed** — this track is READY, not ACTIVE empirical |
...
## G2 — Truth slice has doctrine, not a pinned public run

| Gap | Honest status |
|-----|----------------|
| TruthfulQA / HalluLens not wired as a version-pinned job | Missing |
| Adversarial fluent-lie set of ≥ 40 items | Not built |
| Abstention scoring | Specified, not implemented |
| Independent judge protocol | Specified, not staffed |

Existing strength: anti-hallucination is a founding reason for Ra-Thor. That is **motivation**, not a score.

---

## G3 — Mercy slice has fixtures, not an external bake-off
...
## G4 — Capability suites (WATCH — do not score-claim)

| Suite | Gap |
|-------|-----|
| ARC-AGI-2 / ARC-AGI-3 | No public Ra-Thor or RG leaderboard run |
| GPQA-Diamond / HLE / MMLU-Pro | No pinned eval |
| SWE-bench / LiveCodeBench / Terminal-Bench | No gated agentic run |
| GAIA / OSWorld / WebArena | No long-horizon agent log |
| Constrained self-improvement (Tier 3) | No monitored loop; claiming it would be Band-C violation |

**Rule:** capability numbers without Slice A+B artifacts are marketing, not proof.

---

## G5 — External reproducibility
...
## What would actually improve the combined system
